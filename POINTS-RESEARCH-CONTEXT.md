# Superfluid points research context

Use this map for Superfluid points, SPR campaigns, claim programs, point-event enumeration, claim vouchers, or SUP nonce research. Read only the group relevant to the task, but do not assume an unmentioned file will be loaded automatically.

The general `.agents/skills/superfluid` skill is expected to be installed beside the points skill. Use it for protocol-wide contract ABIs, selectors, subgraph schemas, SDK guides, and generic Superfluid architecture. Those files are intentionally not duplicated here.

## Campaign and endpoint discovery

- `.agents/skills/superfluid-points-research/references/endpoints.md`: operational endpoint catalog, source hierarchy, known IDs, and interpretation rules.
